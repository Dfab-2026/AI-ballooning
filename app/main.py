...
def _write_inspection_sheet(ws, report: InspectionReportPayload, rows: list[InspectionRow], page_no: int, total_pages: int) -> None:
    ...
    ws['M4'] = page_no
    # Clear the 16 characteristic rows without disturbing formatting.
    for row_no in range(12, 28):
        for col in range(1, 7):
            ws.cell(row=row_no, column=col).value = None
    for offset, item in enumerate(rows[:16]):
        row_no = 12 + offset
    ...
    ws['A28'] = 'Remarks: ' + (report.remarks or '')
    ws['A30'] = report.inspected_by or ''
    ws['E30'] = report.qc_incharge or ''
    ws['J30'] = report.approved_by or ''
...
def _build_inspection_workbook(report: InspectionReportPayload, out_path: str) -> None:
    ...
    for _ in range(1, len(chunks)):
        sheets.append(wb.copy_worksheet(source))
