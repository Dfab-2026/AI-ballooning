...
For every drawing, also read these title-block fields when clearly visible: drawing_number, part_name/title, customer/company/customer name, revision, drawing_date, and quantity. Return drawing_date as YYYY-MM-DD when the date is clearly readable. Preserve quantity text such as '2 NOS'. Do not confuse revision, sheet number, date, or BOM item number with drawing_number. If a field is not clearly visible, return null rather than guessing.
...
def _learning_path() -> str:
    ...
    return os.path.join(backend_dir, 'data', 'learning', 'corrections.jsonl')
...
def analyze_batch_with_gemini(image_paths: List[str]) -> List[dict]:
    ...
    for i,(w,h) in enumerate(prepared,1):
        d=by_index.get(i)
        if d is None:
            results.append({'drawing_number':None,'part_name':None,'customer':None,'revision':None,'drawing_date':None,'quantity':None,'characteristics':[],'model':used_model,'error':'No analysis returned for this drawing'})
        else:
            results.append({'drawing_number':(d.drawing_number or '').strip() or None,'part_name':(d.part_name or '').strip() or None,'customer':(d.customer or '').strip() or None,'revision':(d.revision or '').strip() or None,'drawing_date':(d.drawing_date or '').strip() or None,'quantity':(d.quantity or '').strip() or None,'characteristics':_to_output(d,w,h),'model':used_model})
    return results
