...
def export_ballooned_pdf(src: str, out: str, balloons, page_index: int = 0) -> None:
    ...
    if ext == '.pdf':
        ...
        for b in balloons:
            ...
            r = 10
            page.draw_line((x, y), (tx, ty), color=(0.90, 0.12, 0.12), width=.8)
            page.draw_circle((x, y), r, color=(0.90, 0.12, 0.12), fill=(1, 1, 1), width=1.2)
            label = str(b['number'])
            width = fitz.get_text_length(label, fontsize=8)
            page.insert_text((x - width / 2, y + 2.8), label, fontsize=8, color=(0.78, 0.08, 0.08))
        output.save(out)
        output.close()
        source.close()
        return
    ...
    for b in balloons:
        ...
        r = 18
        dr.line((x, y, tx, ty), fill=(220, 35, 35), width=2)
        dr.ellipse((x-r, y-r, x+r, y+r), fill='white', outline=(220, 35, 35), width=3)
        dr.text((x-5, y-7), str(b['number']), fill=(190, 25, 25))
